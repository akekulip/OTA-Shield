"""Generate figures for the 2026-06-06 E19p CLAMPED stochastic 10-trial HW re-run.

Source data: runs/experiments/E19p_stochastic_2026-06-06_clamped/t00-t09/
Aggregate:   runs/experiments/_agg/E19p_stochastic_2026-06-06_clamped.json

Produces:
  runs/figures/E19p_clamped_2026-06-06_per_trial_f1.{pdf,png}
  runs/figures/E19p_clamped_2026-06-06_latency_cdf.{pdf,png}
  runs/figures/E19p_clamped_2026-06-06_confusion_per_trial.{pdf,png}

Latency filter: ATTACK-only hold_digest events (n=correct count, not 227).
Fix applied per review_rigor_2026-06-06.md P3 / Fix 3.
"""
...
EXP_DIR = Path("runs/experiments/E19p_stochastic_2026-06-06_clamped")
...
def fig_per_trial_f1(agg: dict, out: Path) -> None:
    ...
    ax.set_title("E19p Stochastic Rollback (clamped) — per-trial Precision / Recall / F1\n"
                 "(n = 10 trials, Tofino HW, 2026-06-06 re-run)")
...
# -------------------------------------------------------------------------
# Figure 2: detection latency CDF — ATTACK-only events
# -------------------------------------------------------------------------
def fig_latency_cdf(agg: dict, out: Path) -> None:
    ...
    for trial_dir in sorted(EXP_DIR.glob("t*")):
        ...
        # Filter: ATTACK-labeled hold_digest events only.
        # This ensures n matches aggregate JSON (no FP latencies included).
        for d in decisions:
            ...
            if ev is None:
                continue
            # Exclude FP events (LEGIT packets that triggered hold_digest).
    ...
    if not raw_latencies:
        ...
        ax.set_title("E19p HW detection latency (hold_digest, clamped)")
        ax.axvline(stats.get("median", 0) * 1000, color="red",
                   linestyle="--", linewidth=0.8,
                   label=f"median {stats['median']*1000:.1f} ms")
    ...
    ax.set_ylabel("CDF")
    ax.set_ylim(0, 1.05)
    ax.set_title(f"Hold-digest detection latency (ATTACK-only)\n"
                 f"(n = {len(raw_ms)} events, Tofino HW, 2026-06-06 clamped)")
    ax.legend(fontsize=7)
    fig.tight_layout()
    save(fig, out)
...
def fig_confusion_per_trial(agg: dict, out: Path) -> None:
    ...
    ax.set_title("E19p Stochastic Rollback (clamped) — per-trial decision breakdown\n"
                 "(n_legit clamped to n_bms=20, n_events=40 per trial)")
...
# -------------------------------------------------------------------------
def main() -> None:
    agg_path = Path("runs/experiments/_agg/E19p_stochastic_2026-06-06_clamped.json")
    if not agg_path.exists():
        sys.exit(f"Aggregate file not found: {agg_path}")
    agg = json.loads(agg_path.read_text())

    out_base = Path("runs/figures")
    print("Generating E19p clamped 2026-06-06 figures...")

    fig_per_trial_f1(agg, out_base / "E19p_clamped_2026-06-06_per_trial_f1")
    fig_latency_cdf(agg, out_base / "E19p_clamped_2026-06-06_latency_cdf")
    fig_confusion_per_trial(agg, out_base / "E19p_clamped_2026-06-06_confusion_per_trial")
